...
namespace Dilios\Diliosresizeimages;
...
class Repository
{
    ...
    public function installDatabase()
    {
        ...
        $sql[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'dri_configs` (
            `id_dri_config` int(11) NOT NULL AUTO_INCREMENT,
            `related_path` VARCHAR(255) NOT NULL,
            `width` SMALLINT(11) NOT NULL,
            `height` SMALLINT(11) NOT NULL,
            `conserve_original` TINYINT(1) NULL DEFAULT "1",
            `preserve_ratio` TINYINT(1) NULL DEFAULT "1",
            `active` TINYINT(1) NULL DEFAULT "1",
            `date_add` DATETIME NUlL DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY  (`id_dri_config`)
    ...
    {
        $sql = array();
        $sql[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'dri_configs`';
        foreach ($sql as $query) {
            if (Db::getInstance()->execute($query) == false) {
                return false;
            }
    ...
    {
        return $this->module->registerHook('header') && $this->module->registerHook('backOfficeHeader');
    }
